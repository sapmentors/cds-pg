...
describe('OData to Postgres dialect', () => {
    ...
    describe('odata: GET -> sql: SELECT', () => {
        ...
        test('odata: $expand entityset on 1:1 rel -> sql: sub-select from expand-target table', async () => {
            ...
            expect(response.status).toStrictEqual(200)
            expect(response.body.value).toEqual(expect.arrayContaining([expect.objectContaining({ brewery: { ID: 'fa6b959e-3a01-40ef-872e-6030ee4de4e5', name: "Private Landbrauerei Schönram GmbH & Co. KG" } })]))
        })
