...
module.exports = class PostgresDatabase extends cds.DatabaseService {
  ...
  async deploy(model, options = {}) {
    ...
    for (let each of createEntities) {
      ...
      // CASCADE added, because without cascade it will through errors for dependant views.
      if (table) dropTables.push(`DROP TABLE IF EXISTS ${entity} CASCADE`)
      else dropViews.push(`DROP VIEW IF EXISTS ${entity} CASCADE`)
    }
    ...
    if (options.dry) {
      ...
      for (let {
        ...
        log('DROP VIEW IF EXISTS ' + view + ' CASCADE;')
      }
      log()
      for (let {
        DROP: { entity }
      } of dropTables) {
        log('DROP TABLE IF EXISTS ' + entity + ' CASCADE;')
      }
      log()
