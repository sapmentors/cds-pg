...
module.exports = class PostgresDatabase extends cds.DatabaseService {
  constructor(...args) {
    ...
    // Loading connection string from specific environment variable key
    // Special handling for: Heroku postgres
    if(this.options.credentials && this.options.credentials.environmentkey){
      this.options.connectionString = process.env[this.options.credentials.environmentkey]
    }
