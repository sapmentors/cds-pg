...
class PGReferenceBuilder extends ReferenceBuilder {
  ...
  build() {
    ...
    if (this._isFunction()) {
      ...
      if (this._obj.param) {
        ...
        ReferenceBuilder.prototype._parseReference.call(this, this._obj.ref.map(c => this._fromCamalCaseToSnaky(c)))
      }
    } else {
  ...
  _fromCamalCaseToSnaky(name) {
    return name.replace(/[A-Z]/g, (c, offset) => {return (offset == 0 || c.startsWith("_") ? "" : '_') + c.toLowerCase()});
  }
}
