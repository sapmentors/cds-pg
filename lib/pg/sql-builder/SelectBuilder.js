  ...
  _columns(noQuoting) {
    if (this._obj.SELECT.columns) {
      ...
      if (this._obj.SELECT.from.ref[0]) {
        ...
        if (!entity && name.endsWith('.Drafts')) {
          //If is draft entity, search for corresponding entity without suffix
          entity = this._csn.definitions[name.replace('.Drafts', '')]
        }
        ...
        if (entity) {
          for (var prop in entity.elements) {
            if (!this.excludedColumnTypes.includes(entity.elements[prop].type)) {
              ...
              if (name.endsWith('.Drafts') && entity.elements[prop].virtual) {
                //If is draft entity, avoid virtual fileds in query
                continue
              }
