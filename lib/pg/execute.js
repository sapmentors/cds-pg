const { createJoinCQNFromExpanded, rawToExpanded, hasExpand } = require('@sap/cds-runtime/lib/db/expand')
...
const executeInsertCQN = async (model, dbc, cqn) => {
  ...
  results = results.map((result) => remapColumnNames(model.entities[cqn.INSERT.into], result))
  return results
}
