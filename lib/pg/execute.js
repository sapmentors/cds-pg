...
/**
 * Modify the CQN for localization in recursive way.
 *
 * @param {Object} query
 * @param {String} locale
 * @return {void} just modify the cqn;
 */
function _processNestedLocalization(query, locale) {
  query.map(table => {
    if(table.args) _processNestedLocalization(table.args, locale)
    if(table.ref && table.ref[0] && !table.ref[0].match("texts")) {
      let _table = table.ref[0];
      if(table.ref[0].match("localized")) _table = _table.replace('localized.', `localized.${locale}.`);
      else _table = `localized.${locale}.${table.ref[0]}`;
      table.ref[0] = _table;
    }
  })
}
...
/**
 * Modify the CQN for localization according to the users requested locale, except default locale.
 *
 * @param {Object} cqn
 * @param {String} locale
 * @return {void} just modify the cqn;
 */
function _applyLocalization(cqn, locale) {
  if(cqn.SELECT.from.args) {
    // recursive function call for nested table.
    _processNestedLocalization(cqn.SELECT.from.args, locale)
  } else {
    const _table = cqn.SELECT.from.ref[0];
    if(!_table.match("localized")) cqn.SELECT.from.ref[0] = `localized.${locale}.${_table}`;
  }
}
...
function _cqnToSQL(model, cqn, user, locale, txTimestamp, isExpand = false) {
  try {
    const _default_language = cds.env.i18n.default_language;
    // if user requested for others locale, except default locale.
    if(locale != _default_language) {
      let _locale = _default_language;
      if (cds.env.i18n.for_sql && cds.env.i18n.for_sql.includes(locale)) {
        _locale = locale
      }

      // apply the localization logic.
      _applyLocalization(cqn, _locale);
    }
  } catch (e) {}
