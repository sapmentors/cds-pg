...
const { processExpand, processCQN } = require('../execute')
...
async function handleRead(dbc, cqn, model) {
  ...
  if (cqn.SELECT.count) {
    if (cqn.SELECT.limit) {
      ...
      return Promise.all([
        processCQN(dbc, countQuery, model),
        processCQN(dbc, cqn, dbc),
      ]).then(([countResult, result]) => _arrayWithCount(result, countResult[0].counted))
    } else {
      return processCQN(dbc, cqn, model).then((result) => _arrayWithCount(result, result.length))
    }
  }
  ...
  return processCQN(dbc, cqn, model)
}
